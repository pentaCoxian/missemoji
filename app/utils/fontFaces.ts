 ...
 * unicode-range subsets the text touches, at the current weight); for uploaded
 * fonts the raw bytes; system families need nothing. Missing sources (the
 * `<link>` fallback path, or the family not loaded yet) mean a worker could not
 * match the preview, so the caller should render on the main thread instead.
 */
export function collectFontFaces(project: EmojiProject): CollectedFaces {
  const family = project.font.family
  const custom = useFontsStore().customFont(family)
  if (custom) {
    // plain object: the record lives in a reactive store and proxies cannot be cloned
    return {
      faces: [{ family, weight: custom.weight, style: custom.style, data: custom.data }],
      needsFaces: true,
      hasFaces: true,
    }
  }
